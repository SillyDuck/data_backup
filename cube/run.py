...
m_name = 'v3sMulPDRcubeAna_all_ori'
script = 'ver pdr p1 -s\n'
...
for ll in allcases:
    ...
    w.write( 'q -f' );
    w.close();
    command = Command('./v3mul -f '+m_name+'dofile.tmp');
    ...
    if(ret == -15):
        ...
        for j,ggg in enumerate(rF_all):
            if ggg.startswith('CubeSize'):
                aaa = ggg.strip().split()
                result.write( cn + ',' + aaa[2] )
            elif ggg.startswith('Inductive'):
                aaa = ggg.split()
                result.write( ',' + 'UNSAT' + ',' + aaa[6] + ',' + aaa[9] + ',');
                for k in range(1,7):
